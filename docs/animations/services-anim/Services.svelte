<script lang="ts">
    ...
    let { selectedSlug: initialSlug = null }: { selectedSlug?: string | null } =
        $props();
    ...
    const selectedService = $derived(
        selectedSlug ? getServiceBySlug(selectedSlug) : null,
    );
    ...
    onMount(() => {
        ...
        const unsubStates = animationCoordinator.cardStates.subscribe(
            (value) => {
                cardStatesMap = value;
            },
        );
...
>
    <div class="container base-grid">
        ...
        <div class="services-grid" class:has-selection={hasSelection}>
            {#each displayedServices as service, i (service.slug)}
                ...
                {@const shouldHide =
                    hasSelection && !isSelected && animPhase === "complete"}
    ...
    <div role="status" aria-live="polite" aria-atomic="true" class="sr-only">
        {#if selectedService}
            Now viewing {selectedService.title} details. Press Escape to return to
            all services.
        {:else}
